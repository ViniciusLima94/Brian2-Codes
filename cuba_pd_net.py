...
eqs = '''
	dv/dt = ((-v + v_r)/tau_m + (Ie+Ii)/Cm) : volt (unless refractory)
...
for c in range(0, 8):
    for r in range(0, 8):
        ...
        if (c % 2) == 0:
            if c == 2 and r == 0:
                con.append( Synapses(p[c],p[r], """ w:amp """,on_pre = 'Ie_post += 2*w') )
                con[-1].connect(condition='i!=j', p=table[r][c])
                con[-1].w = 'clip((w_ex + std_w_ex*randn()),w_ex*0.1, w_ex*10.0)'
            else:
                con.append( Synapses(p[c],p[r], """ w:amp """,on_pre = 'Ie_post += w') )
                con[-1].connect(condition='i!=j', p=table[r][c])
                con[-1].w = 'clip((w_ex + std_w_ex*randn()),w_ex*0.1, w_ex*10.0)'
            con[-1].delay = 'clip(d_ex + std_d_ex*randn(),0,d_ex*10)'
        # Inhibitory layer
        else:
            con.append( Synapses(p[c],p[r], """ w:amp """,on_pre = 'Ii_post -= g*w') )
            con[-1].connect(condition='i!=j', p=table[r][c])
            con[-1].w = 'clip((w_ex + std_w_ex*randn()),w_ex*0.1, w_ex*10.0)'
...
bg_in  = []
for r in range(0, 8):
	bg_in.append( PoissonInput(p[r], 'Ie', bg_layer[r], 8*Hz, weight = w_ex ) )
