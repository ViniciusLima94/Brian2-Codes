...
w_ex = rf*87.8*pA       # Excitatory weight
std_w_ex = rf*8.8*pA    # Standard deviation weigth
g = 4.0                 # Inhibitory weight balance
tau_syn = 0.8*ms        # Post-synaptic current time constant

w_thal = 2*rf*87.8*pA       # Excitatory weight
...
eqs = '''
	dv/dt = ((-v + v_r)/tau_m + I/Cm) : volt (unless refractory)
    I : amp
'''
...
for c in range(0, 8):
    for r in range(0, 8):
        ...
        if (c % 2) == 0:
            if c == 2 and r == 0:
                con.append( Synapses(p[c],p[r], """ w:amp """,on_pre = 'v_post += 2*(w/Cm)*1*ms') )
                con[-1].connect(condition='i!=j', p=table[r][c])
                con[-1].w = 'clip((w_ex + std_w_ex*randn()),w_ex*0.1, w_ex*10.0)'
            else:
                con.append( Synapses(p[c],p[r], """ w:amp """,on_pre = 'v_post += (w/Cm)*1*ms') )
                con[-1].connect(condition='i!=j', p=table[r][c])
                con[-1].w = 'clip((w_ex + std_w_ex*randn()),w_ex*0.1, w_ex*10.0)'
            con[-1].delay = 'clip(d_ex + std_d_ex*randn(),0,d_ex*10)'
        # Inhibitory layer
        else:
            con.append( Synapses(p[c],p[r], """ w:amp """,on_pre = 'v_post -= g*(w/Cm)*1*ms') )
            con[-1].connect(condition='i!=j', p=table[r][c])
            con[-1].w = 'clip((w_ex + std_w_ex*randn()),w_ex*0.1, w_ex*10.0)'
...
bg_in  = []
for r in range(0, 8):
	bg_in.append( PoissonInput(p[r], 'v', bg_layer[r], 8*Hz, weight = (w_ex/Cm)*1*ms ) )
...
thal_con = []
for r in range(0,8):
    thal_con.append(Synapses(thal_input,p[r],""" w:amp """,on_pre = 'v_post += (w/Cm)*1*ms'))
